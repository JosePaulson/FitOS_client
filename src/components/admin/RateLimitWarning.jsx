...
/**
 * Mounted once, globally (see AdminLayout.jsx). Whenever any API call
 * anywhere in the app gets a 429, the axios interceptor dispatches a
 * 'fitos:rate-limited' event instead of logging the user out — this shows
...
export default function RateLimitWarning() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[70] flex justify-center px-4 pt-3 pointer-events-none">
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl shadow-lg pointer-events-auto max-w-sm bg-card border border-yellow-500/30">
        <span className="text-base shrink-0">⏳</span>
        <p className="text-xs text-cream">{message}</p>
        <button onClick={() => { clearTimeout(timerRef.current); setMessage(null) }}
          className="ml-1 text-sm leading-none shrink-0 text-muted hover:text-cream">×</button>
      </div>
    </div>
  )
}
